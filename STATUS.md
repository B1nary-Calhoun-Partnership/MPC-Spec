...
- **bsv-mpc MessageBox transport — #2 part 1 of 5 + part 2 of 5 + part 3 of 5 absorbed into `main`** (no PR; impl-only Calhoun-side work, gated by live-relay proof). Commits [`815156a`](https://github.com/B1nary-Calhoun-Partnership/bsv-mpc/commit/815156a) (scaffold + wire), [`17ea329`](https://github.com/B1nary-Calhoun-Partnership/bsv-mpc/commit/17ea329) (BRC-31 auth + HTTP routes + first live-relay proof), [`21b6cd3`](https://github.com/B1nary-Calhoun-Partnership/bsv-mpc/commit/21b6cd3) (**/ws WebSocket subscribe with backfill + heartbeat + reconnect per §06.4 + §06.12**; live-relay proof extended to cover WS push + backfill-on-reconnect, both byte-exact). Crate `bsv-mpc-messagebox` now ships `subscribe`/`WsSubscription`/`InboundEnvelopeEvent` at the root; parts 4 and 5 (public `MessageBoxClient` API with `leaveRoom` cleanup, bridge integration, within-stack mainnet ceremony e2e) remain.
...
- `bsv-mpc` CI fully gated: build + clippy strict (-D warnings) + workspace tests + doctests + wasm32. Node 24 opt-in ahead of GH's 2026-06-02 forced switch. fmt locked. **390 tests green** (up from 367 at PR #1; +19 from the messagebox transport + WS subscribe work — BRC-104 byte-shape vectors, server-event parser, `next_backoff` §06.12 sequence, ws URL scheme swap, ENV-gated live-relay scenarios).
...
|---|---|---|---|
| **John** (Calhoun) | M1 #2 MessageBox transport (auth + send + list + ack + **WS subscribe live-proven**; remaining: public `MessageBoxClient` API with `leaveRoom` cleanup, bridge integration, mainnet ceremony e2e) | #4 presig lifecycle (~600-900 LOC) · #6 deploy 1 cosigner · #J1 cggmp21-fork org transfer/visibility | Ishaan #9 (only blocks #4) |

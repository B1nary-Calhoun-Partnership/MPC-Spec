...
- **`<binary-messagebox-host-tbd>`** (Binary's deployed MessageBox) supports WebSocket via its parent Node.js implementation.
- **`bsv-messagebox-cloudflare`** (Calhoun's CF Worker port) deliberately deleted WebSocket support in v2 (`MessageRoom` Durable Object class deleted) to match `go-messagebox-server`'s HTTP-only scope. CLAUDE.md states: *"HTTP REST only, no WebSocket. Clients poll `/listMessages` or rely on FCM push."*

Federation (per cosigner pinning their preferred relay) was the original design. With `bsv-messagebox-cloudflare` HTTP-only, federating produces an asymmetric experience: cosigners pinning `<binary-messagebox-host-tbd>` get sub-second signing; cosigners pinning `bsv-messagebox-cloudflare` get poll-bound latency (5 Hz hardcoded in `MessageBoxCosignerTransport`).

The user (Calhoun) confirmed unlimited resources for the partnership and explicitly committed to making `bsv-messagebox-cloudflare` WebSocket-compliant if WebSocket is the right answer.

## Decision

WebSocket (Socket.IO/EngineIO compatible) is the **canonical receive transport** for v1. Both production relays support it:

- `<binary-messagebox-host-tbd>` — already supports it.
- `bsv-messagebox-cloudflare` — Calhoun extends it with Socket.IO over CF Worker Durable Objects in Phase 1, restoring parity with `<binary-messagebox-host-tbd>`'s WS surface.
...
The cost is on Calhoun's side: ~1500 LOC to add Socket.IO/EngineIO compatibility over CF Worker Durable Objects. With unlimited resources framing, this is acceptable. Reverses the v2-scope decision in `bsv-messagebox-cloudflare` deliberately — that decision was constraint-driven (match go-messagebox-server scope), and the unlimited-resources framing makes the constraint inapplicable.
...
- **`bsv-messagebox-cloudflare`:** Add Socket.IO/EngineIO over CF Worker Durable Objects. ~1500 LOC. Add `MessageRoom` (or equivalent) Durable Object class back. Phase 1 work item.
...
This ADR reverses the implicit position from the first swarm's `SWARM-CONVERGENCE.md` recommendation that `bsv-messagebox-cloudflare` stay HTTP-only. The reversal is correct in light of unlimited-resources framing.

## Alternatives considered

- **Keep `bsv-messagebox-cloudflare` HTTP-only; users pinning it get slow ceremonies** — rejected; asymmetric UX undermines federation. The 5Hz polling floor (~800ms sign-no-presig) is unacceptable for the Notary product.
